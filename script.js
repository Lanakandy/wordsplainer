// script.js - ENHANCED VERSION WITH IMPROVED GRAPH GENERATION
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Enhanced pan function with smoother transitions
    ...
    function handleDockClick(event) {
        ...
        switch (button.dataset.action) {
            case 'meaning': generateGraphForView('meaning'); break;
            case 'synonyms': generateGraphForView('synonyms'); break;
            case 'opposites': generateGraphForView('opposites'); break;
            case 'derivatives': generateGraphForView('derivatives'); break;
            case 'collocations': generateGraphForView('collocations'); break;
            case 'context': generateGraphForView('context'); break;
            case 'idioms': generateGraphForView('idioms'); break;
            case 'translation': 
                if (languageModal) languageModal.classList.add('visible'); 
                break;
    ...
    function handleZoomControlsClick(event) {
        ...
        switch (button.dataset.action) {
            case 'zoom-in': newScale = Math.min(currentTransform.k * 1.3, 5); break;
            case 'zoom-out': newScale = Math.max(currentTransform.k / 1.3, 0.1); break;
            case 'zoom-reset': newScale = 1; break;
        }
    ...
    function createInteractiveText(d3TextElement, text, onWordClick) {
        d3TextElement.selectAll("*").remove();
        const lines = text.split('\n');
        lines.forEach((line, lineIndex) => {
            const words = line.split(/(\s+)/);
            const tspan = d3TextElement.append("tspan")
                .attr("x", 0)
                .attr("dy", lineIndex === 0 ? "0.3em" : "1.2em");
    ...
    function handleMouseOver(event, d) {
        if (tooltip && d.text) {
            ...
            tooltip.textContent = d.explanation || d.text;
    ...
    async function toggleTranslationExamples(translationNode) {
        ...
        if (examplesShown) {
            ...
            for (const [eng, trans] of Object.entries(translationNode.exampleTranslations)) {
                ...
                // Position examples around the translation node
                const angle = angleOffset * (Math.PI / 3);
                const exNode = { 
                    id: exId, 
                    text: `${eng}\n${langTrans}`, 
                    type: 'example', 
                    sourceNodeId: translationNode.id, 
                    clusterId: translationNode.clusterId, 
                    visible: true,
                    x: translationNode.x + Math.cos(angle) * 80,
                    y: translationNode.y + Math.sin(angle) * 80
                };
    ...
    function updateCentralNodeState() {
        graphGroup.selectAll('.central-node')
            .attr("class", d => `node central-node ${d.clusterId === currentActiveCentral ? 'active-central' : ''}`);
    }

    function updateActiveButton() {
        controlsDock.querySelectorAll('button').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.action === currentView);
        });
    }
    ...
    function toggleFullScreen() {
        if (!document.fullscreenElement) {
            graphContainer.requestFullscreen().catch(console.error);
        } else {
    ...
    function saveAsPng() {
        ...
        img.onload = () => {
            ...
            link.download = `wordsplainer-${currentActiveCentral || 'graph'}.png`;
    ...
    // Enhanced drag behavior with constraints for rectangular nodes
    function dragstarted(event, d) {
        if (!event.active) simulation.alphaTarget(0.1).restart();
        d.fx = d.x;
        d.fy = d.y;
        
        // Store original position for rectangular nodes
        if (['meaning', 'example', 'context', 'idioms'].includes(d.type)) {
            d._originalX = d.x;
            d._originalY = d.y;
        }
    }
    ...
    function dragged(event, d) {
        ...
        if (textBoxTypes.includes(d.type)) {
            // Constrained dragging for rectangular nodes - only allow small movements
            const maxDistance = 50;
            const dx = event.x - d._originalX;
            const dy = event.y - d._originalY;
            ...
            if (distance <= maxDistance) {
                ...
                d.fx = d._originalX + dx * scale;
                d.fy = d._originalY + dy * scale;
            }
        } else {
            // Free dragging for circular nodes
            d.fx = event.x;
            d.fy = event.y;
        }
    }
    ...
    function dragended(event, d) {
        ...
        if (textBoxTypes.includes(d.type)) {
            // Snap back rectangular nodes if they're too far from center
            const cluster = graphClusters.get(d.clusterId);
            if (cluster) {
                const centralNode = cluster.nodes.find(n => n.isCentral);
                if (centralNode) {
                    const distance = Math.sqrt((d.x - centralNode.x) ** 2 + (d.y - centralNode.y) ** 2);
                    if (distance > NODE_SPACING * 1.5) {
                        // Snap back with animation
                        const angle = Math.atan2(d.y - centralNode.y, d.x - centralNode.x);
                        d.fx = centralNode.x + Math.cos(angle) * NODE_SPACING;
                        d.fy = centralNode.y + Math.sin(angle) * NODE_SPACING;
                    }
                }
            }
        }
